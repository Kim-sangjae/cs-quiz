...
_1차 시도 — pg_trgm (실패)_  
`제1정규형(1NF)의 조건은?` vs `"모든 속성값이 원자값(Atomic Value)이어야 한다" 몇 정규형에 해당하나요?`  
같은 개념이지만 표현이 완전히 달라 트라이그램 유사도 0.05 수준 → 임계값 0.1에서도 탐지 불가.  
pg_trgm은 문자 수준 비교라 동의어·역방향 표현을 처리할 수 없음이 실제 사례로 확인됨.

_2차 시도 — pgvector + 문제 텍스트만 임베딩 (실패)_  
임베딩 도입 후에도 코사인 유사도가 0.40 수준으로 낮아 임계값 설정 불가.  
원인: `제1정규형(1NF)의 조건은?`처럼 짧은 질문 텍스트는 맥락이 부족해 임베딩 품질 저하.  
부작용: `트랜잭션의 ACID 속성 중 원자성(Atomicity)의 의미는?`(0.418)이 실제 유사 문제인 1NF 문제(0.402)보다 높게 나오는 오탐 발생 — "원자" 단어가 겹치는 표면적 유사도가 의미 유사도를 앞섬.

_3차 시도 — 문제 + 정답 텍스트 결합 임베딩 (현재 채택)_  
`"${question} ${correctAnswer}"` 형태로 결합 후 임베딩.  
`제1정규형(1NF)의 조건은? 모든 속성값이 원자값(Atomic Value)이어야 한다` → 쿼리 문장과 핵심 키워드 "원자값(Atomic Value)"이 공유되어 유사도 0.70으로 상승.  
임계값 0.5 적용 시 안정적으로 탐지, 관련 없는 문제와 명확히 구분됨.
