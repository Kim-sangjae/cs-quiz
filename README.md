...
[비로그인]
  홈(/) ─── 랭킹 확인, 오늘의 문제 열람
  게시판(/board) ─── 목록·상세 열람 (등록·좋아요·신고 불가)
  결과 공유 링크(/result/[id]) ─── 열람 가능
  퀴즈 공유 링크(/quiz/play?sharedFrom=) ─── 열람 가능 (제출 불가)
  퀴즈 시작 시도 → 로그인 페이지로 리다이렉트

[로그인]
  Google / 카카오 OAuth
    └── 닉네임 설정 (신규 가입)
          └── 홈(/)
                ├── 퀴즈(/quiz) ──── 카테고리·모드 선택 → 풀기 → 결과
                │                                     └── 오답 → 복습 스케줄 등록
                ├── 오늘의 문제 ─── 1문제 풀기 → 전체 참여자 정답률 공개
                ├── 대결(/battle) ── 친구 초대 → 실시간 1:1 퀴즈 진행
                ├── 마이페이지 ───── 통계·오답·뱃지·북마크·주간목표
                ├── 게시판 ─────────  열람 + 문제 등록 + 좋아요·신고
                ├── 친구(/friends) ─ 친구 추가·대결 초대·실시간 상태
                └── 문의(/inquiry) ─ 버그·건의 등록
```
